import type { AssistantBlock, SessionEvent, SharePayload, ShareRound } from "./types.js";

function mergeAssistantText(blocks: AssistantBlock[]): AssistantBlock[] {
...
function finalizeRound(round: ShareRound | null): ShareRound | null {
  if (round === null) {
    return null;
  }
...
export function dropLatestRound(events: SessionEvent[]): SessionEvent[] {
  let lastUserIndex = -1;
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case "commentary":
        break;
      default:
        ((unreachable: never) => unreachable)(event);
    }
  }
