...
function pushAssistantText(events, text, timestamp) {
...
function parseCodexJsonl(raw, fallbackSessionId) {
    const lines = raw
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
    ...
    const emitToolSummary = (callId, fallbackName, result) => {
        if (!callId) {
            events.push({
                type: "tool_summary",
                tool: fallbackName,
    ...
    for (const record of records) {
        ...
        if (record.type === "event_msg") {
            ...
            if (payloadType === "exec_command_end") {
                emitToolSummary(typeof payload.call_id === "string" ? payload.call_id : undefined, "exec_command", typeof payload.aggregated_output === "string" ? payload.aggregated_output : undefined);
            }
            continue;
        }
        ...
        if (payloadType === "function_call_output") {
            emitToolSummary(typeof payload.call_id === "string" ? payload.call_id : undefined, "tool", typeof payload.output === "string" ? payload.output : undefined);
        }
    }
    if (events.length === 0) {
...
export async function loadSessionDocument(inputPath) {
...
async function findSessionFileByThreadId(rootDir, threadId) {
...
export async function resolveCurrentCodexSessionPath() {
