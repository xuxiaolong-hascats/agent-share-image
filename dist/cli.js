...
import { buildSharePayload, dropLatestRound, extractCompleteRounds, extractSharePayload, } from "./session.js";
import { renderPayloadToPng, renderPayloadToSvg } from "./renderer.js";
function printUsage() {
    console.log("Usage: agent-share-image (--input <session.json|session.jsonl> | --current) [--round <n> | --select <expr> | --list] [--limit <n>] [--output <file.png|file.svg>]");
}
...
function truncatePreview(text, maxChars = 14) {
...
function getAssistantPreview(blocks) {
...
function printRoundList(rounds, startIndex = 0) {
...
function parseSelectExpression(expression, maxIndex) {
...
async function main() {
    ...
    const payload = args.select
        ? buildSharePayload(parseSelectExpression(args.select, rounds.length).map((index) => rounds[index]), parsed.sessionId)
